...
import { getNewAccessToken } from "./gmailSetup.js";
...
export async function sendPaymentViaGmail(
  ...
  if (!refreshToken) {
    logger.warn(
      "Gmail refresh token not set. Run OAuth setup at /api/admin/gmail-auth-url to generate one.",
    );
    return;
  }
  ...
  try {
    ...
    await transporter.sendMail({
      from: `Groeax Payments <${adminEmail}>`,
      to: adminEmail,
      subject: `[Groeax] Payment ${statusLabel} - ${info.userName} - ${info.amount} USDT`,
      html: htmlBody,
    });

    logger.info({ adminEmail }, "Payment notification sent via Gmail API");
  } catch (err) {
    logger.error({ err, adminEmail }, "Failed to send Gmail notification");
  }
}
