...
function MetricCard({ label, value, sub, icon: Icon, color = "text-foreground", index }: { label: string; value: string; sub?: string; icon: React.ElementType; color?: string; index: number; }) {
  return (
    <motion.div custom={index} initial="hidden" animate="show" variants={FADE_UP} className="glass-card p-4 flex flex-col gap-1.5">
...
function MonthlyGoalCard({ monthlyPnL, goal, onSetGoal }: { monthlyPnL: number; goal: number; onSetGoal: (g: number) => void; }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18 }} className="glass-card p-4 flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setDraft(String(goal || "")); setEditing(true); }} className="text-muted-foreground hover:text-foreground transition-colors p-1 rounded hover:bg-accent"><Pencil className="w-3 h-3" /></button>
      </div>
      {editing && (
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setEditing(false)} className="p-1.5 rounded hover:bg-accent text-muted-foreground transition-colors"><X className="w-3.5 h-3.5" /></button>
        </div>
      )}
      {goal > 0 ? (
  ...
  return (
    <div className="p-5 space-y-5">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">{analytics.totalTrades} trades tracked</p>
      </div>
      <OpenPositions />
      <LiveTradingSignals />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-card p-4 lg:col-span-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass-card p-4">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="glass-card p-4">
      ...
      {analytics.totalTrades > 0 && (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="glass-card p-4">
              ...
              <tbody>
                {[...trades].filter((t) => t.date && !isNaN(new Date(t.date).getTime())).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 8).map((t) => (
                  <tr key={t.id} className="border-b border-border/40 hover:bg-accent/30 transition-colors">
                    ...
                    <td className="py-2 text-right"><span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${t.outcome === "WIN" ? "bg-emerald-500/20 text-emerald-400" : t.outcome === "LOSS" ? "bg-red-500/20 text-red-400" : "bg-muted text-muted-foreground"}`}>{t.outcome ?? "—"}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
