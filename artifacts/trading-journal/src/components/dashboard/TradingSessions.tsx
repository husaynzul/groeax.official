...
import { SESSION_HOURS_UTC, TradingSession } from "@/types";
...
const SESSIONS: {
  ...
  { id: "ASIA",     label: "Asia",     flag: "🌏", color: "text-cyan-400",   glow: "shadow-cyan-500/40",   startUTC: 23, endUTC: 8  },
  { id: "TOKYO",    label: "Tokyo",    flag: "🗼", color: "text-violet-400", glow: "shadow-violet-500/40", startUTC: 0,  endUTC: 9  },
  { id: "LONDON",   label: "London",   flag: "🇬🇧", color: "text-blue-400",   glow: "shadow-blue-500/40",   startUTC: 8,  endUTC: 17 },
  { id: "NEW_YORK", label: "New York", flag: "🗽", color: "text-emerald-400", glow: "shadow-emerald-500/40",startUTC: 13, endUTC: 22 },
];
...
function sessionProgress(startUTC: number, endUTC: number, utcHour: number, utcMin: number): number {
  let totalMins = (endUTC > startUTC ? endUTC - startUTC : 24 - startUTC + endUTC) * 60;
  let elapsed = 0;
  if (startUTC <= endUTC) {
          ...
          return (
            ...
            >
              ...
              <p className="text-[9px] text-muted-foreground/60 mt-0.5 font-mono">
                {String(s.startUTC).padStart(2, "0")}:00 – {String(s.endUTC).padStart(2, "0")}:00
              </p>
